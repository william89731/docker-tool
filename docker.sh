...
CHECK() {
  {
    ...
    for IMAGE in $IMAGES; do
      ...
      if [ "$LATEST" != "$VERSION" ]; then
        printf '%s' "\n$IMAGE" >>/tmp/images.txt
        printf '%s' "\n$LATEST" >>/tmp/images.txt
      fi
